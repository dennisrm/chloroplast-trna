...
def identityCounter(sequence,consensus,outdic):
    ...
    first = True
    id = 0
    consensusNoGaps = [consensus[i] for i in range(len(consensus)) if consensus[i] != "-"]
    for key, value in sequence.items():

        seq = "".join(value)
        seq = "".join([seq[i] for i in range(len(consensus)) if consensus[i] != "-"])
        if first:
            first = False
            bases = ["A", "C", "G", "T", "-"]
            baseIndex = {"A": 0, "C": 1, "G": 2, "T": 3, "-": 4}
            countarray = np.zeros((len(seq)), dtype=int)
